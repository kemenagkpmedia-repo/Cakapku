...
import { Target, Plus, Pencil, Trash2, AlertCircle, RefreshCw, ChevronRight, FileText } from 'lucide-react';
      ...
      {isLoadingPerkins ? (
        ...
        <div className="space-y-8">
          {filteredPerkins.map((perkin) => (
            <div key={perkin.id} className="space-y-4">
              <div className="flex items-center gap-3 px-4">
                <div className="p-2 bg-accent/10 rounded-lg">
                  <FileText className="w-5 h-5 text-accent" />
                </div>
                <div>
                  <h2 className="text-lg font-bold text-text-header tracking-tight">{perkin.nama_perkin}</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {(perkin.sasaran_kegiatans || []).map((sk) => (
                  ...
                  >
                    <Card className="rounded-2xl border-border hover:shadow-lg transition-all group overflow-hidden">
                ...
                {(perkin.sasaran_kegiatans || []).length === 0 && (
                  <div className="col-span-full py-8 px-6 bg-slate-50/50 rounded-2xl border border-dashed border-border text-center">
